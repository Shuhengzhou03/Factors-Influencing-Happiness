---
title: "What Makes Us Happy? Examining the Impact of Social, Economic, and Personal Factors"
...
- **`tidyverse`** [@tidyverse]: Provided a suite of tools for data manipulation and visualization, significantly simplifying the workflow.
...
- **`brms`** [@brms]: A flexible modeling tool built on Stan, used to perform Bayesian regression analysis on the data.
...
#| tbl-cap: "Sample Preview of Happiness Data: A snapshot showing key variables"
...
#| echo: false
#| warning: false
#| label: fig-MaritalStatusDistribution
...
#| echo: false
#| warning: false
#| label: fig-ChildrenDistribution
#| fig-cap: "Distribution of the Number of Children by Happiness Level: Showing Family Size Across Different Happiness Categories"
...
```
@fig-ChildrenDistribution illustrates the relationship between the number of children and happiness levels, with data points grouped by happiness categories for better clarity. The violin plots display the density distribution of the number of children across the three happiness levels: "Not Too Happy," "Pretty Happy," and "Very Happy," differentiated by distinct shades of green. Additionally, jittered points overlay the violin plots, representing individual observations, thereby providing a detailed view of the raw data.

The chart highlights that the number of children is distributed similarly across happiness levels, with the highest density of observations clustering around smaller family sizes. A few data points stand out as outliers, representing families with larger numbers of children. This combination of violin plots and jittered points effectively showcases both the overall trends and individual variances, making the relationships between family size and happiness levels visually accessible and easy to interpret.
...
#| echo: false
#| warning: false
#| label: fig-EducationHappinessDistribution
#| fig-cap: "Proportion of Happiness Levels by Education Level: Representing the Distribution of Happiness Categories Across Different Levels of Educational Attainment"
...
#| echo: false
#| warning: false
#| label: fig-JobSatisfactionHappiness
#| fig-cap: "Proportion of Happiness Levels by Job Satisfaction: Depicting the Distribution of Self-Reported Happiness Categories Across Different Job Satisfaction Levels"
...
#| echo: false
#| warning: false
#| label: fig-IncomeHappiness
#| fig-cap: "Real Income and Happiness Level: This analysis explores the distribution of real income across different happiness categories, using jitter points to illustrate the variability of individual data within each category"
...
Although the Bayesian logistic regression model performs well in analyzing the relationship between happiness and its predictors, it still has certain limitations. First, the model assumes a linear relationship between the predictors and the log-odds of happiness, which may overlook complex non-linear or interaction effects between variables. For example, the relationship between age and happiness may exhibit non-linear patterns, leading to potential biases in estimation. Second, the imbalanced distribution of categories in the data—where the "very happy" category has fewer samples—may cause the model to favor predicting the majority classes (e.g., "not too happy" and "pretty happy") while underperforming in classifying the minority class. Lastly, the model employs weakly informative priors (e.g., Normal(0, 2) and Cauchy(0, 2)) to ensure flexibility. However, in cases with small samples or rare categories, these priors may impact the stability of coefficient estimates, increasing uncertainty in the results.

## Alternative Models Considered

The Bayesian logistic regression model was chosen due to its unique ability to incorporate prior knowledge, quantify uncertainty, and provide robust parameter estimates in the analysis of complex individual-level happiness data. Unlike traditional logistic regression, the Bayesian framework allows for the inclusion of weakly informative priors, which help stabilize estimates in the presence of limited or imbalanced data (e.g., the "very happy" category being underrepresented) while avoiding overfitting. This is particularly advantageous in handling imbalanced target variable distributions, where the Bayesian model performs better in classifying minority categories. Additionally, the Bayesian model computes posterior distributions for each predictor, offering a comprehensive understanding of the uncertainty and variability in the estimated effects. This probabilistic framework not only enhances interpretability but also supports better decision-making by providing credible intervals instead of single-point estimates.
...
For these reasons, the Bayesian logistic regression model stands out for its flexibility and robustness, capturing complex non-linear relationships and potential interactions while quantifying uncertainty. It offers more accurate and comprehensive insights for predicting happiness compared to alternative models, making it a superior choice for this analysis.
...
## Coefficient Summary {#sec-coefficient-summary}
...
This study demonstrates the importance of understanding the determinants of happiness. Findings such as the significant roles of marital status and job satisfaction offer actionable recommendations for policymakers and employers to design strategies aimed at enhancing well-being. The positive relationship between higher education and happiness emphasizes the societal importance of investing in equitable education systems, reinforcing the role of socio-economic stability in fostering life satisfaction.
...
## Filtering Irrelevant Responses
...
As shown in @fig-stanareyouokay-2, the Rhat values are close to 1, further confirming that the chains have likely converged and the model results are reliable.
...
- **Email**: [shuheng.zhou@mail.utoronto.ca](mailto:shuheng.zhou@mail.utoronto.ca)
